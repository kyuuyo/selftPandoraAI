...
const renderer = {
    code(code, lang) {
        ...
        const copyButton = `
<button
    class="absolute top-0 right-0 flex items-center gap-1 pt-1 pr-2 font-sans whitespace-normal"
    data-is-copy-button="true"
>
    <svg
        class="relative w-3 h-3 top-px"
    ...
    <span class="copy-status">Copy</span>
        ...
        return `<pre class="relative p-0">${copyButton}<code class="hljs${language ? ` language-${language}` : ''}">${highlightedCode}</code></pre>`;
    },
};
marked.use({ renderer });
...
const sendMessage = async (input, parentMessageId = null) => {
    ...
    if (!stream) {
        const result = await fetch(`${config.apiBaseUrl}`, {
            ...opts,
    ...
    try {
        await fetchEventSource(`${config.apiBaseUrl}`, {
            ...opts,
            openWhenHidden: true,
            signal: processingController.value.signal,
            onopen(response) {
...
<template>
    ...
    <div class="relative flex flex-col items-center flex-grow">
        ...
        <div
            ...
            class="w-full px-3 pb-12 overflow-y-auto rounded-sm"
            style="overflow: overlay;"
        >
            <TransitionGroup name="messages">
                <div
                    class="w-full max-w-4xl mx-auto message"
                ...
                >
                    ...
                    >
                        ...
                        <!-- message text -->
                        <div
                            class="max-w-6xl prose-sm prose break-words prose-chatgpt"
        ...
        <div
            ...
            class="absolute left-0 right-0 z-10 flex flex-row w-full max-w-4xl px-3 mx-auto xl:px-0 mb-7 sm:mb-0"
        >
            <div class="relative flex flex-row items-stretch justify-center w-full rounded shadow">
                <div
                    ref="chatButtonsContainerElement"
                    class="absolute flex items-stretch justify-center gap-2 mb-3 bottom-full"
                    :class="{ 'w-full': !processingController }"
                >
                    <button
                        v-if="processingController"
                        @click="stopProcessing"
                        class="flex-1 px-5 py-2 text-sm transition duration-300 ease-in-out rounded shadow bg-white/10 text-slate-300 hover:bg-white/20"
                    >
                        Stop
                    </button>
                    <button
                        ...
                        class="flex-1 px-3 py-2 text-sm transition duration-300 ease-in-out rounded shadow bg-white/10 text-slate-300 hover:bg-white/20"
                    >
                        {{ response }}
                    </button>
                </div>
                ...
                <button
                    ...
                    class="absolute top-0 bottom-0 left-0 z-10 flex items-center justify-center w-10 h-10 my-auto ml-2"
                >
                    <Transition name="fade" mode="out-in">
                        <GPTIcon
                            v-if="activePresetNameToUse === 'chatgpt' || activePresetToUse?.client === 'chatgpt'"
                            class="block w-10 h-10 p-2 transition duration-300 ease-in-out rounded-lg cursor-pointer hover:bg-black/30 hover:shadow"
                            :class="{
                ...
                <textarea
                    ...
                    class="w-full py-4 rounded-l-sm resize-none pl-14 pr-14 text-slate-100 bg-white/5 placeholder-white/40 focus:outline-none placeholder:truncate"
                    :class="{
                ...
                <button
                    ...
                    title="Regenerate"
                    class="flex items-center flex-1 px-4 transition duration-300 ease-in-out text-slate-300 bg-white/5 hover:bg-white/10"
                >
                ...
                <button
                    ...
                    :disabled="!!processingController"
                    class="flex items-center flex-1 px-4 transition duration-300 ease-in-out rounded-r-sm text-slate-300 bg-white/5"
                    :class="{
                        'opacity-50 cursor-not-allowed': !!processingController,
